...
pub enum Key {
    ...
    // Additional OS specific keys for scancodes

    Alt,     // alt key on Linux and Windows (option key on macOS)
    Command, // command key on macOS (super key on Linux, windows key on Windows)
    Control, // control key
    Meta,    // meta key (also known as "windows", "super", and "command")
    Option,  // option key on macOS (alt key on Linux and Windows)
    Shift,   // shift key
    Super,   // super key on linux (command key on macOS, windows key on Windows)
    Windows, // windows key on Windows (super key on Linux, command key on macOS)
